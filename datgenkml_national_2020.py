...
# Name Code: Converting CSV files of the national point source emission inventory to KML files 
...
def extractAddress(row):
  ...
  return '%s,%s,%s' % (row['CVEIDEN'],row['Coordinates'],row['Address'],row['Industry_Name'],row['State'],row['Fuel_Type'] )
...
def main():
  ...
  else: order = ['CVEIDEN','Coordinates','Address','Industry_Name','State','Fuel_Type']
  csvreader = csv.DictReader(open('C:/Users/Abraham/Desktop/emission_pointsource_sample.csv'),order)
  kml = createKML(csvreader, 'C:/Users/Abraham/Desktop/datgen_pointsource.kml',order)
